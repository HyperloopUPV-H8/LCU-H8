...
namespace LCU{
	...
	public:
		...
		MovingAverage<400> z_error_filter;
		MovingAverage<400> z_position_filter;
...
//			specific_state_machine.add_enter_action([&](){
//				control.position_control.set_ems_K();
//			}, STABLE);
		}
...
//							tcp_timeout = true;
			...
			general_state_machine.add_low_precision_cyclic_action([&](){
				if(not tcp_handler.SLAVE_CONNECTION.is_connected()) tcp_handler.SLAVE_CONNECTION.reconnect();
			}, 1ms);
