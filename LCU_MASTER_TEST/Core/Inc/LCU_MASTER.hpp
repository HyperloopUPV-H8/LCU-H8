...
namespace LCU{
	...
	void change_frequency_lpu_validation(){
		LCU_MASTER<LPU_VALIDATION>::toggle_led();
		LCU_MASTER<LPU_VALIDATION>::lcu_master->actuators.change_frequency(LCU_MASTER<LPU_VALIDATION>::lcu_master->incoming_orders_handler.frequency);
		Time::set_timeout(500, LCU_MASTER<LPU_VALIDATION>::toggle_led);
	}
	...
	void stop_levitation_vehicle_5dof(){
		...
		for(TimedAction*& control_action : LCU_MASTER<VEHICLE_5DOF>::lcu_master->control_current_actions){
			LCU_MASTER<VEHICLE_5DOF>::lcu_master->state_machine_handler.general_state_machine.remove_cyclic_action(control_action);
		}
		LCU_MASTER<VEHICLE_5DOF>::lcu_master->control_current_actions.clear();
	...
	void test_individual_current_control_5dof(){
		LCU_MASTER<VEHICLE_5DOF>::toggle_led();
		COIL_ID target_coil = LCU_MASTER<VEHICLE_5DOF>::lcu_master->incoming_orders_handler.coil_target;
		float reference_current = LCU_MASTER<VEHICLE_5DOF>::lcu_master->incoming_orders_handler.reference_current;

		if(LCU_MASTER<VEHICLE_5DOF>::lcu_master->data.reference_currents[target_coil] != 0){
			ErrorHandler("Current control is already running");
			return;
		}
		if(Actuators<VEHICLE_5DOF>::is_coil_from_master(target_coil)){
			LCU_MASTER<VEHICLE_5DOF>::lcu_master->actuators.turn_on();
			LCU_MASTER<VEHICLE_5DOF>::lcu_master->control.change_current_reference(target_coil, reference_current);
			TimedAction* control_action = LCU_MASTER<VEHICLE_5DOF>::lcu_master->state_machine_handler.general_state_machine.add_mid_precision_cyclic_action([&,target_coil](){
				LCU_MASTER<VEHICLE_5DOF>::lcu_master->control.execute_current_control(target_coil);
			}, 500us);
			LCU_MASTER<VEHICLE_5DOF>::lcu_master->control_current_actions.push_back(control_action);
		}else{
			LCU_MASTER<VEHICLE_5DOF>::lcu_master->outgoing_orders_handler.coil_id = target_coil;
			LCU_MASTER<VEHICLE_5DOF>::lcu_master->outgoing_orders_handler.reference_current = reference_current;
			LCU_MASTER<VEHICLE_5DOF>::lcu_master->tcp_handler.send_to_slave(LCU_MASTER<VEHICLE_5DOF>::lcu_master->outgoing_orders_handler.test_slave_current_control_order);
		}
		LCU_MASTER<VEHICLE_5DOF>::lcu_master->data.reference_currents[target_coil] = reference_current;
		Time::set_timeout(500, LCU_MASTER<VEHICLE_5DOF>::toggle_led);
	}

	void turn_on_lateral_levitation(){
		LCU_MASTER<VEHICLE_5DOF>::toggle_led();
		if(LCU_MASTER<VEHICLE_5DOF>::lcu_master->state_machine_handler.general_state_machine.current_state == GeneralStateMachine<VEHICLE_5DOF>::OPERATIONAL &&
				LCU_MASTER<VEHICLE_5DOF>::lcu_master->state_machine_handler.specific_state_machine_handler.specific_state_machine.current_state != SpecificStateMachine<VEHICLE_5DOF>::IDLE){
			LCU_MASTER<VEHICLE_5DOF>::lcu_master->control.position_control.set_ems_K();
		}else{
			ErrorHandler("Cannot Start Lateral Levitation without vertical levitation");
		}
		Time::set_timeout(500, LCU_MASTER<VEHICLE_5DOF>::toggle_led);
	}
